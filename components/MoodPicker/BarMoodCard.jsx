...
function RestaurantMoodCard() {
  const { barMood } = useContext(RestaurantContext);
  ...
  const stackOffset = 10;
  const maxOffset = 60;
  const expandedHeight = 180 + (barMood.length - 1) * stackOffset;
  const collapsedHeight = expandedHeight + barMood.length * maxOffset;
  ...
  return (
    <Animated.View style={[styles.container, { height: containerHeight }]}>
      ...
      <View style={styles.cardContainer}>
        {barMood.map((moodItem, index) => (
          <TouchableWithoutFeedback key={index} onPress={() => navigation.navigate('HomeScreen')}>
            <Animated.View style={[
              styles.card,
              getDynamicCardStyle(index, barMood.length)
            ]}>
              <Text style={styles.CardTitle}>{moodItem}</Text>
              <Text style={styles.CardSubTitle}>Une sélection de bar {moodItem}</Text>
            </Animated.View>
